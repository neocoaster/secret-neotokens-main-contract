use crate::msg::{ContractInfo, Token};
...
pub fn credits_history_storage<S: Storage>(store: &mut S) -> Bucket<S, u64> {
    bucket(BUCKET_USER_CREDITS, store)
}

pub fn credits_history_storage_read<S: Storage>(store: &S) -> ReadonlyBucket<S, u64> {
